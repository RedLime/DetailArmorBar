...
public class CustomArmorBar {
    ...
    public void draw(ItemStack itemStack, DrawContext context, int xPos, int yPos, boolean isHalf, boolean isMirror) {
        ...
        if (isHalf) {
            InGameDrawer.drawTexture(renderInfo.getTexture(), context, xPos, yPos, renderInfo.getTextureOffsetHalf().x, renderInfo.getTextureOffsetHalf().y,
                    renderInfo.getTextureWidth(), renderInfo.getTextureHeight(), renderInfo.getColor(), isMirror);
        } else {
            InGameDrawer.drawTexture(renderInfo.getTexture(), context, xPos, yPos, renderInfo.getTextureOffsetFull().x, renderInfo.getTextureOffsetFull().y,
    ...
    public void drawOutLine(ItemStack itemStack, DrawContext context, int xPos, int yPos, boolean isHalf, boolean isMirror, Color color) {
        ...
        if (isHalf) {
            if (renderInfo instanceof ItemBarRenderManager) {
                InGameDrawer.drawTexture(renderInfo.getTexture(), context, xPos + 4, yPos, offset.x + 4, offset.y, 5, 9,
                        renderInfo.getTextureWidth(), renderInfo.getTextureHeight(), color, false);
            } else {
                InGameDrawer.drawTexture(renderInfo.getTexture(), context, xPos, yPos, offset.x, offset.y,
