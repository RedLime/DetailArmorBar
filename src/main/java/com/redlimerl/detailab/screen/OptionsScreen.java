...
public class OptionsScreen extends Screen {
    ...
    protected void init() {
        ...
        ButtonWidget features = addButton(new ButtonWidget(width / 2 - 92, height / 6 + 140, 60, 20,
                new TranslatableText("option.detailarmorbar.title.features"), (matrixStack) -> {
            if (client != null) {
                client.openScreen(new OptionsScreen(parent, OptionType.FEATURES));
            }
        }));
        features.active = optionType != OptionType.FEATURES;

        ButtonWidget animation = addButton(new ButtonWidget(width / 2 - 30, height / 6 + 140, 60, 20,
                new TranslatableText("option.detailarmorbar.title.animation"), (matrixStack) -> {
            if (client != null) {
                client.openScreen(new OptionsScreen(parent, OptionType.ANIMATION));
            }
        }));
        animation.active = optionType != OptionType.ANIMATION;

        ButtonWidget etc = addButton(new ButtonWidget(width / 2 + 32, height / 6 + 140, 60, 20,
                new TranslatableText("option.detailarmorbar.title.etc"), (matrixStack) -> {
            if (client != null) {
                client.openScreen(new OptionsScreen(parent, OptionType.ETC));
            }
        }));
